...
2. **Follow what applies.** Open only the notes the task needs (`memory_read`); search (`memory_search`, also with `project_dir`) when the task touches something the Index doesn't show, including topics.
