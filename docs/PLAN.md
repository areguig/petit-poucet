...
10. **0.2.1, released 2026-09-25** after the owner's tests in Claude Code, Copilot CLI, VS Code, IntelliJ and the Copilot app: one plugin folder for every client, self-locating MCP and hook commands, a `memory` skill, required `project_dir`, search-all fallback, projects learning their git remote, agents' built-in memory ruled out.
11. **Next:** see §13, Roadmap.
...
- Copilot in IDEs (VS Code, IntelliJ): **done on `dev`, 2026-09-25.** One `plugin/` folder for every client, like ponytail: Claude Code reads `.claude-plugin/`, Copilot CLI reads `plugin.json` (hooks, MCP bootstrap and agent in `copilot/`, all self-locating), and Copilot's IDE hosts read it as a Claude plugin. Verified: Copilot CLI, VS Code and the Copilot app load memory by hook (VS Code even shows the pebble line; per ~/.copilot/session-state the app's session-start hook returned the context); IntelliJ runs our session-start hook with no output (refused), so there the `memory` skill or `memory_index` loads it: 1 miss out of 5 IntelliJ tests, where reading the README looked enough to the model. A two-folder split was tried and dropped: no clear gain, and duplicated files. Also found in IntelliJ: an agent searched without `project_dir`, so the project's notes were filtered out; `project_dir` is now required, a search from an unknown folder covers every note, and projects created without a checkout (by `migrate`) learn their git remote the first time their folder is matched.
