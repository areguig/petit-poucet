...
- Copilot in IDEs (VS Code, IntelliJ): **done on `dev`, 2026-09-25.** IntelliJ loads CLI-installed plugins but refuses every plugin hook ("untrusted workspace folder"), passes `${PLUGIN_ROOT}` unexpanded, drops MCP server instructions, and reads any folder holding `.claude-plugin/` as a Claude plugin (an Agent Plugins manifest didn't change that). Fix, tested in IntelliJ: a separate Copilot-format folder `copilot-plugin/` with a self-locating bootstrap, plus a `memory` skill and a `memory_index` description telling the agent to load memory first; it loaded memory unprompted in 2 of 2 memory-only tests. Verified per client on 2026-09-25: Claude Code and Copilot CLI load memory by hook; VS Code runs the Copilot hooks too (without `PLUGIN_ROOT`, hence self-locating hook commands) and got the context at session start; the Copilot app and IntelliJ load it through the skill/tool (the app called `memory_read` unprompted). No stop reminder in IntelliJ or, as far as seen, the app.
