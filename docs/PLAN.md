...
- Copilot in IDEs (VS Code, IntelliJ): **done on `dev`, 2026-09-25.** One `plugin/` folder for every client, like ponytail: Claude Code reads `.claude-plugin/`, Copilot CLI reads `plugin.json` (hooks, MCP bootstrap and agent in `copilot/`, all self-locating), and Copilot's IDE hosts read it as a Claude plugin. Verified: Copilot CLI and VS Code load memory by hook (VS Code even shows the pebble line); the Copilot app and IntelliJ load it through the `memory` skill or `memory_index` (IntelliJ refuses plugin hooks): 1 miss out of 5 IntelliJ tests, where reading the README looked enough to the model. A two-folder split was tried and dropped: no clear gain, and duplicated files.
